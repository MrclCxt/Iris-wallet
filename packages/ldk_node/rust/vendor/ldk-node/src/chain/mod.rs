...
impl ChainSource {
	...
	// PATCH IRIS: forca a proxima sincronizacao a ser uma varredura completa da arvore de
	// enderecos, para reencontrar fundos em indices que este aparelho ainda nao revelou.
	pub(crate) fn request_full_scan(&self) {
		if let ChainSourceKind::Esplora(esplora_chain_source) = &self.kind {
			esplora_chain_source.request_full_scan();
		}
	}
