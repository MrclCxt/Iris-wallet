...
impl From<Network> for Currency {
	fn from(network: Network) -> Self {
		match network {
			...
			// PATCH Iris: testnet4 usa o MESMO HRP da testnet3 no BOLT11
			// (`lntb`) — o BOLT11 não define prefixo próprio para ela.
			//
			// Sem isto, cai no `_` abaixo: em debug o `debug_assert!` dá panic
			// (quebra a criação de qualquer fatura), e em RELEASE é pior — o
			// assert some e a fatura sai como Regtest (`lnbcrt`), inválida
			// para quem for pagar. Bug de verdade do lightning-invoice 0.34.1.
