...
pub(super) struct EsploraChainSource {
	...
	force_full_scan: std::sync::atomic::AtomicBool,
}
...
impl EsploraChainSource {
	pub(crate) fn new(
		...
		Self {
			...
			logger,
			node_metrics,
			force_full_scan: std::sync::atomic::AtomicBool::new(false),
		}
	}

	// PATCH IRIS: agenda uma varredura completa no proximo sync, mesmo que a carteira ja tenha
	// sincronizado antes. Sem isso um aparelho que ja fez a primeira sincronizacao rasa fica
	// preso nela para sempre, porque a decisao olha so se existe timestamp de sync anterior.
	pub(crate) fn request_full_scan(&self) {
	...
	async fn sync_onchain_wallet_inner(&self, onchain_wallet: Arc<Wallet>) -> Result<(), Error> {
		...
		let forced = self.force_full_scan.swap(false, std::sync::atomic::Ordering::SeqCst);
		let incremental_sync = !forced
		...
		if incremental_sync {
			...
			let wallet_sync_timeout_fut = tokio::time::timeout(
				Duration::from_secs(BDK_WALLET_FULL_SCAN_TIMEOUT_SECS),
