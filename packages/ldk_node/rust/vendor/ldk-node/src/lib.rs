...
impl Node {
	...
	pub fn sync_wallets(&self) -> Result<(), Error> {
		...
		self.sync_wallets_inner()
	}

	/// PATCH IRIS: varre a arvore de enderecos inteira (ate o gap limite) em vez de conferir
	/// apenas os enderecos que este aparelho ja revelou.
	///
	/// Necessario porque "quais indices ja usei" e estado LOCAL, nao derivavel da semente: dois
	/// aparelhos restaurados com as mesmas palavras podem enxergar saldos diferentes se um deles
	/// nunca revelou o indice onde o dinheiro esta. Uma varredura completa reconcilia os dois.
	pub fn full_scan_wallets(&self) -> Result<(), Error> {
		if !*self.is_running.read().unwrap() {
			return Err(Error::NotRunning);
		}

		self.chain_source.request_full_scan();
		self.sync_wallets_inner()
	}

	fn sync_wallets_inner(&self) -> Result<(), Error> {
